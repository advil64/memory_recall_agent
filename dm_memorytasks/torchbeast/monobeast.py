...
from torchbeast.dm_memorytasks import EnvironmentSettings, load_from_docker
import numpy as np
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    model = Net(env.observation_space.shape, len(env.action_space), flags.use_lstm)
    ...
    learner_model = Net(
        env.observation_space.shape, len(env.action_space), flags.use_lstm
...
def test(flags, num_episodes: int = 10):
    ...
    gym_env = create_env(flags)
    env = environment.Environment(gym_env)
    model = Net(gym_env.observation_space.shape, len(gym_env.action_space), flags.use_lstm)
    model.eval()
...
class PsychLab(gym.Env):
    def __init__(self, env_config):
        ...
        env_settings = EnvironmentSettings(seed=123, level_name='spot_diff_extrapolate')
        
        self.env = load_from_docker(name='gcr.io/deepmind-environments/dm_memorytasks:v1.0.1', settings=env_settings)        
        ...
        self.observation_space = observation_spec['RGB_INTERLEAVED']
